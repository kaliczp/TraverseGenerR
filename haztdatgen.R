...
hazcimkoorhoz <- function(x, file, settlement = "Sehonna", objkod = 105201, student = "Kis Pista", teacher = "Kalicz Péter", parcels = 3, addcoo = TRUE) {
    act.date <- Sys.Date()
    ## Data preparation
    ## Create sf
    elso.mat <- as.matrix(rbind(x[1:4,1:3], x[1,1:3]))
    row.names(elso.mat) <- c((1:4),11)
    elso.poly <- st_polygon(list(elso.mat))
    x_sfc <- st_sfc(elso.poly, crs=23700)
    ## calculate centroids
    x.centr <- st_coordinates(st_centroid(x_sfc))
    x.centr <- cbind(x.centr, x[1:3,3])
    x.centr <- round(x.centr, 3)
